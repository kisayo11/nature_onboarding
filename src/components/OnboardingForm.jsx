...
const OnboardingForm = ({ googleAppsScriptUrl, selfService = false }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    empBirth: params.get('birth') || '',
    empPhone: params.get('phone') || ''
  })
  ...
  const [viewedDocs, setViewedDocs] = useState({ training: false, privacy: false })
  ...
  const docTemplates = {
    ...
    privacy: '13b98fzAIaf1UtNVmlqqBFyLWQPMDmlnheIKp4jDBoUk'   // 개인정보(입사)
  }
  ...
  const handleSendSmsLink = async () => {
    const { empJob, empDept, empName, empBirth, empPhone } = formData
    if (!empName || !empPhone) {
  ...
  const handleConfirm = () => {
    const { empJob, empDept, empName, empBirth, empPhone } = formData
    if (!empJob || !empDept || !empName || !empBirth || !empPhone) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`❌ 서류 제출 처리 중 오류가 발생했습니다: ${error.message}`)
    } finally {
    ...
    <article className="step-card visible">
      <div className="step-header">
        ...
        <div className="step-title-wrapper">
          <h2>신규입사자 전자서명 제출</h2>
          <p className="step-desc">배치전 집합 교육 서류 확인 및 모바일 간편 서명</p>
        </div>
      </div>

      <div className="step-body">
        <div style={{ background: 'rgba(16, 185, 129, 0.05)', padding: '1.25rem', borderRadius: '12px', border: '1px solid rgba(16, 185, 129, 0.2)', position: 'relative' }}>
          <span style={{ position: 'absolute', top: '-1px', right: '-1px', backgroundColor: 'var(--primary-light)', color: 'white', padding: '0.2rem 0.6rem', borderRadius: '0 12px 0 8px', fontSize: '0.75rem', fontWeight: 'bold' }}>
            실시간
          </span>
          <h4 style={{ marginBottom: '0.75rem', color: '#047857', fontSize: '1.05rem' }}>
            <i className="ph-fill ph-chalkboard-teacher"></i> 배치전 집합 교육 / 개인정보 보호 서약서 작성
          </h4>
          <p style={{ fontSize: '0.85rem', color: '#065F46', marginBottom: '1rem' }}>
            두 가지 필수 입사 서류를 모두 탭하여 확인하신 후, 하단 인적 사항을 입력해 전자서명을 완료해 주세요.
          </p>
          ...
          <form onSubmit={handleSubmit} style={{ marginTop: '1.5rem' }}>
            ...
            {allDocsViewed && !isConfirmed && (
              <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
                <button type="button" className="btn-primary" onClick={handleConfirm} style={{ background: 'linear-gradient(135deg, #059669, #047857)' }}>
            ...
            {isConfirmed && (
              <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px dashed rgba(16, 185, 129, 0.3)' }}>
                {!selfService && (
                ...
                {!selfService && sigMethod === 'mobile' && (
                  <div style={{ textAlign: 'center', padding: '1.5rem', border: '1px solid #cbd5e1', borderRadius: '12px', backgroundColor: '#f8fafc', marginBottom: '1rem' }}>
                    <h4 style={{ marginBottom: '1rem', color: '#0F766E', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                      <i className="ph-fill ph-cell-tower"></i> 대면 모바일 서명 진행
                    </h4>
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                      입사자분은 아래 QR 코드를 본인 휴대폰으로 스캔하여 서명을 진행해 주세요.
                    </p>
                    <div style={{ display: 'inline-block', padding: '10px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '1rem' }}>
                      <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(
                          `${window.location.origin}${window.location.pathname}?view=mobile-sign&type=onboarding&name=${encodeURIComponent(formData.empName)}&birth=${encodeURIComponent(formData.empBirth)}`
                        )}`}
                    ...
                    {isPolling ? (
                      <p style={{ fontSize: '0.85rem', color: '#0F766E', fontWeight: 'bold' }}>
                ...
                {!selfService && sigMethod === 'sms' && (
                  <div style={{ textAlign: 'center', padding: '1.5rem', border: '1px solid #cbd5e1', borderRadius: '12px', backgroundColor: '#f8fafc', marginBottom: '1rem' }}>
                    ...
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1.5rem' }}>
                      입사자의 연락처({formData.empPhone})로 서약 서류 작성 링크를 발송합니다.<br />
                      발송 후 입사자가 직접 모바일로 작성을 제출하면 즉시 자동 아카이빙 처리됩니다.
                    </p>
                    <button
                      type="button"
                      ...
                      style={{ background: 'linear-gradient(135deg, #0F766E, #14B8A6)', width: 'auto', padding: '0.5rem 1.5rem', margin: '0 auto' }}
                    >
