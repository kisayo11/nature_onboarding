...
const RegistrationForm = ({ googleAppsScriptUrl }) => {
  ...
  useEffect(() => {
    if (!['mobile', 'sms'].includes(sigMethod) || !allConsented || !formData.name || birthKey.length !== 6 || !googleAppsScriptUrl) {
      setIsPolling(false)
      return undefined
    }
  ...
  }, [sigMethod, allConsented, formData.name, birthKey, googleAppsScriptUrl])
  ...
  return (
    <>
      ...
      <header className="header register-header">
        ...
        <p>기초 인사 정보 등록 및 필수 입사 서류 확인·전자서명을 한 번에 진행합니다.</p>
      </header>
          ...
          <div className="register-details">
            ...
            <section className="register-section">
              ...
              <div className="form-group">
                ...
                <textarea id="opinion" name="opinion" value={formData.opinion} onChange={updateField} rows="3" placeholder="기타 전달사항 또는 의견" />
              </div>
            </section>

            {/* 필수 서약 및 교육 확인 (원내 규정 명칭 준수) */}
            <section className="register-section">
              <h2>입사 필수 서류 확인 및 전자서명</h2>
              <p className="consent-guide">
                원내 규정에 따른 3가지 필수 서류를 확인하신 후, 하단에서 1회 전자서명을 진행해 주세요.
              </p>
            ...
            >
              {isSubmitting ? '제출 및 문서 생성 중...' : '인사기록카드 및 입사서약 일괄 제출'}
            </button>
          </div>
        </form>
      </main>
